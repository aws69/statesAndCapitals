...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        List<StateInfo> lastFiveStates = states.stream().skip(Math.max(0, states.size() - 5)).limit(5).collect(toList());
        ...
        Long statesNumber = Long.valueOf(states.stream().collect(collectingAndThen(counting(), Long::intValue)));
        ...
        StateInfo cardinalState = states.stream().filter(state -> "cardinal".equalsIgnoreCase(state.getStateBird())).findAny().orElseThrow(() -> new NoSuchElementException("No state with a cardinal bird found"));
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(state -> state.getStateAnthem() != null && !state.getStateAnthem().isEmpty());
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(state -> state.getStateMotto() != null && state.getStateMotto().split("\\s+").length == 1);
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(x -> x.getCapital().getAverageYearlyPrecipitationInInches()).sum();
        ...
        Map<String, Long> numberOfStatesByTimeZone =states.stream().collect(groupingBy(x->x.getTimeZones().toString(),counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(groupingBy(x->x.getCapital().getTimeZone(), counting()));
        ...
        Integer maxStateElevation = states.stream().map(StateInfo::getHighestElevationInFeet).max(Comparator.naturalOrder()).orElseThrow(() -> new NoSuchElementException("No state elevations found"));
        ...
        LocalDate earliestDateStateEnteredUnion = states.stream().map(StateInfo::getDateAdmittedToUnion).min(LocalDate::compareTo).orElseThrow(() -> new NoSuchElementException("No entry dates found"));
        ...
        StateInfo stateWithLeastDistanceBetweenHighAndLowPoints = states.stream().min(Comparator.comparingDouble(state -> state.getHighestElevationInFeet() - state.getLowestElevationInFeet())).orElse(null);
        ...
        Map<String, String> stateNameToCapitalNamesMap = states.stream().collect(toMap(StateInfo::getStateName, x->x.getCapital().getCapitalName()));
        ...
        Long totalNumberOfHonoluluSisterCitiesStartingWithCa = states.stream().filter(state -> "Honolulu".equals(state.getCapital().getCapitalName())).flatMap(honolulu -> honolulu.getCapital().getSisterCities().stream()).filter(city -> city.startsWith("Ca")).count();
        ...
        List<String> countriesOfTheWorldWithNoUSCapitalSisterCities = Arrays.stream(allCountriesList).filter(allCountriesList -> states.stream().flatMap(state -> state.getCapital().getSisterCities().stream()).noneMatch(city -> Boolean.parseBoolean(allCountriesList))).collect(toList());
